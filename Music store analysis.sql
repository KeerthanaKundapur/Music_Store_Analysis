...
--6)Write query to return the email,first name,last name and Genre of all Rock Music listeners. Return your
...
--8)Return all the track names that have a song length longer than the average song length. Return the Name
...
--10)We want to find out the most popular music genre to each country. We determine the most popular genre
--as the genre with the highest amount of purchases. Write a query that returns each country along with
--the top Genre.For Countries where the maximum number of purchases is should return all Genres.
...
--where the top amount spent is shared, provided all customers who spent this amount.
